...
export const createEmployee = async (req: MulterRequest, res: Response) => {
  try {
    const {
      ...
      userEmail,
      email,
      photo, // <--- ✅ FIXED: Destructure 'photo' as sent by frontend
      faceData,
      panNumber,
      aadhaarNumber,
      bankAccount,
...
export const getAllEmployees = async (req: Request, res: Response) => {
  try {
    ...
    const where: Prisma.EmployeeWhereInput = {
      ...
      ...(department ? { department: { name: { equals: department as string, mode: "insensitive" } } } : {}),
...
export const updateEmployee = async (req: MulterRequest, res: Response) => {
  try {
    ...
    if (updateData.photo && !req.file) {
      const photo = updateData.photo as string;
      const base64Data = photo.replace(/^data:image\/\w+;base64,/, "");
      const buffer = Buffer.from(base64Data, "base64");
      const fileName = `uploads/${Date.now()}-employee-photo.jpg`;
      fs.writeFileSync(fileName, buffer);
...
/* ... */
  try {
    ...
    const qrBuffer = await QRCode.toBuffer(qrData, { type: 'png', margin: 1, width: 80 });
...
<svg width="448" height="322" viewBox="0 0 448 322" fill="none" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink">
  ...
  <g clip-path="url(#clip4_0_1)">
    ...
    const idCardBuffer = await sharp(svgBuffer)
      .png({ quality: 100 })
      .toBuffer();
