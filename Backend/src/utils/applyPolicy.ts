...
function toTime(base: Date, timeStr: string | null): Date | null {
...
export function applyPolicyLogic(
  inTime: Date | null,
  outTime: Date | null,
  policy: AttendancePolicy | null
) {
  if (!policy) return { status: "PRESENT", deduction: 0 };
  if (!inTime) return { status: "ABSENT", deduction: policy.deductionFullDay };
