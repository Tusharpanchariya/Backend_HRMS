...
export const createPayroll = async (req: Request, res: Response) => {
  try {
    ...
    const period: string | undefined = req.body.period;
    const paymentDateInput: string | Date | undefined = req.body.paymentDate || req.body.date;
...
async function generateBarcodeDataURL(payloadText: string) {
...
const sanitize = (s: string) =>
  s
    ...
    .replace(/_+/g, "_")
    .substring(0, 120);
export const generateBulkPayrollReceipts = async (req: Request, res: Response) => {
  try {
    ...
    for (const record of records) {
      ...
      const empName =
        (employee.fullName || employee.full_name || employee.name || employee.firstName || employee.email) ?? `emp_${employee.id}`;
        ...
        sections: [
          {
            ...
            children: [
              new Paragraph({
                children: [new TextRun({ text: "Payroll Receipt", bold: true, size: 32 })],
              }),
              new Paragraph({ children: [new TextRun({ text: `Payroll ID: ${record.id}`, bold: false })] }),
              new Paragraph({ children: [new TextRun({ text: `Employee Name: ${empName}` })] }),
              new Paragraph({ children: [new TextRun({ text: `Employee ID: ${record.employeeId}` })] }),
              new Paragraph({ children: [new TextRun({ text: `Email: ${employee.email ?? "N/A"}` })] }),
              new Paragraph({ children: [new TextRun({ text: `Phone: ${employee.phone ?? employee.mobile ?? "N/A"}` })] }),
              new Paragraph({ children: [new TextRun({ text: " " })] }),
              new Paragraph({ children: [new TextRun({ text: `Base Salary: ₹${(record.baseSalary ?? 0).toFixed(2)}` })] }),
              new Paragraph({ children: [new TextRun({ text: `Allowances: ₹${(record.allowances ?? 0).toFixed(2)}` })] }),
              new Paragraph({ children: [new TextRun({ text: `Bonus: ₹${(record.bonus ?? 0).toFixed(2)}` })] }),
              new Paragraph({ children: [new TextRun({ text: `Incentive: ₹${(record.incentive ?? 0).toFixed(2)}` })] }),
              new Paragraph({ children: [new TextRun({ text: `Advance Payment: ₹${(record.advancePayment ?? 0).toFixed(2)}` })] }),
              new Paragraph({ children: [new TextRun({ text: `Deductions: ₹${(record.deductions ?? 0).toFixed(2)}` })] }),
              new Paragraph({ children: [new TextRun({ text: " " })] }),
              new Paragraph({
                children: [new TextRun({ text: `Net Salary: ₹${(record.netSalary ?? 0).toFixed(2)}`, bold: true })],
              }),
              new Paragraph({ children: [new TextRun({ text: `Payment Date: ${record.paymentDate ? new Date(record.paymentDate).toLocaleDateString() : "N/A"}` })] }),
              new Paragraph({ children: [new TextRun({ text: " " })] }),
              new Paragraph({ children: [new TextRun({ text: "This is a computer generated salary receipt." })] }),
            ],
          },
        ],
      });
...
export const generatePayrollReceiptPDF = async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
    ...
    <div class="section">
      <div class="row">
        ...
        <div>
          ...
          <div class="value">${(employee as any).phone || (employee as any).mobile || "—"}</div>
